package LANraragi::Plugin::Metadata::ComicInfo_fix;
...
sub get_tags {
    ...
    if ( $path_in_archive ) {
        ...
        if (defined $result) {
            $type_tags = $result->text;
        }
        $result = Mojo::DOM->new->xml(1)->parse($stringxml)->at('Series');
        if (defined $result) {
            $series_tags = $result->text;
        }
        $result = Mojo::DOM->new->xml(1)->parse($stringxml)->at('Characters');
        if (defined $result) {
            $character_tags = $result->text;
        }
        $result = Mojo::DOM->new->xml(1)->parse($stringxml)->at('EHTags');
        if (defined $result) {
            $ehtags = $result->text;
        }
        #Delete local file
        ...
        @found_tags = try_add_tags(\@found_tags, "misc:", $type_tags);
        @found_tags = try_add_tags(\@found_tags, "genre:", $genre);
        @found_tags = try_add_tags(\@found_tags, "series:", $series_tags);
